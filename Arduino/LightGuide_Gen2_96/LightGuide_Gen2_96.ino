...
/* Date: 10/29/2018                              */
/* Modify by: YunlinTG (sef96121@gmail.com)      */
...
bool hexToBytes(const char* hex, uint8_t* out, size_t out_len) {
  // Two characters per byte, length must be exactly out_len*2
  size_t n = strlen(hex);
...
/* Set LEDs once using a 96-bit mask (LSB-first, row-major: bit0=A01, ... bit95=H12) */
bool applyMaskHex(const char* maskHex) {
  ...
  // Strategy here: Unselected items are cleared as black, selected items are set to led_color.
  FastLED.clear();
...
void restart(uint32_t wait_ms = 100) {
  ...
  Serial.flush();             // Send out as many sequence messages as possible
  delay(wait_ms);             // Delay some time
  ESP.restart();              // Restart
  while (true) { delay(1); }  // This should never be executed; insurance block
}
...
void parseData() {
  ...
  if (strtokIndx) strcpy(rowLetter, strtokIndx);
  else rowLetter[0] = '\0';
  strtokIndx = strtok(NULL, ",");  // column (1-based)
  if (strtokIndx) columnNumber = atoi(strtokIndx);
  strtokIndx = strtok(NULL, ",");  // command
  if (strtokIndx) strcpy(illuminationCommand, strtokIndx);
  else illuminationCommand[0] = '\0';
  strtokIndx = strtok(NULL, ",");  // barcode
  if (strtokIndx) strcpy(plateBarcode, strtokIndx);
  else plateBarcode[0] = '\0';
  // Optional fields: R, G, B, [bright] (if not provided, defaults to -1)
  strtokIndx = strtok(NULL, ",");
  if (strtokIndx) r = strtol(strtokIndx, NULL, 10);
  strtokIndx = strtok(NULL, ",");
  if (strtokIndx) g = strtol(strtokIndx, NULL, 10);
  strtokIndx = strtok(NULL, ",");
  if (strtokIndx) b = strtol(strtokIndx, NULL, 10);
  strtokIndx = strtok(NULL, ",");
  if (strtokIndx) bri = strtol(strtokIndx, NULL, 10);
  // If R/G/B values are provided, update the color (no command restrictions)
  if (r >= 0 && g >= 0 && b >= 0) {
  ...
  // Brightness modification is only permitted during L commands.
...
bool parseIlluminationCommand(const char* cmd) {
  ...
  if (strcmp(cmd, "X") == 0) {
    ...
    // Here, plateBarcode is used as the 24Hex mask carrier (the 4th field in parseData)
