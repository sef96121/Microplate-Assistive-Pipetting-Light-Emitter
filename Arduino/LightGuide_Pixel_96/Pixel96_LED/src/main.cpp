...
const byte numChars = 128;              // determines the number of characters for the lists: receivedCharArray,tempStorage
char receivedCharArray[numChars] = {0}; // Stores the character input received from the user

/* Components of command received over serial port - row, column and illumination command */
char rowLetter[3] = {0};            // Stores a single character, that is later used to determine the target row that the user wants to light-up
char plateBarcode[33] = {0};
int rowNumber = 0;                  // used to store the usable-index-number-value obtained with targetIndex, so that targetIndex can be reset to -1 so the convertRowLetterToNumber() keeps working
...
char illuminationCommand[4] = {0};  // Stores a single character, that is later used to determine whether the user wants to light-up a row, a column, or a single bulb
...
int pixelNumber = 0;           // Wall number
uint8_t bright = 255;      // LED bright setting, default=255

uint32_t rowExposureTimeMs = 1000; // Row exposure time in milliseconds, default=1000ms
...
void loop() {
  ...
  if (newData == true){
    ...
    bool commandParsed = parseIlluminationCommand(illuminationCommand);
    if (commandParsed) {
      if (commandSupportsRowExposure(illuminationCommand)) startRowExposure();
      else exposureScanActive = false;
      sendAck();
    }
    else sendErr(F("BAD_CMD_OR_MASK"));
    newData = false;
  }
  if (exposureScanActive && rowExposureTimeSpecified) {
  ...
  }else if (exposureScanActive) {
    unsigned long now = millis();
    // 防止溢位問題：如果 lastRowSwitchMillis > now（溢位後），重置為 now
    if (lastRowSwitchMillis > now) {
        lastRowSwitchMillis = now;
    }
    if (now - lastRowSwitchMillis >= 1) { // 每行曝光至少 1ms，避免過快切換導致無法觀察到
        lastRowSwitchMillis = now;
        currentScanRow++;
        if (currentScanRow >= numRows) {
            currentScanRow = 0;
...
void illuminationTest() {
  exposureScanActive = false;
  uint8_t originalBright = leds.getBrightness();
  leds.clear();
  Serial.println(F("LED test start!!"));
  leds.setBrightness(min<uint8_t>(originalBright ? originalBright : 32, 32));
  // Test each row
  leds.testRow();
  leds.off();
  leds.setBrightness(originalBright); // Restore original brightness
  Serial.println(F("LED test down!!"));
}
...
bool applyMaskHex(const char *maskHex) {
  ...
  uint8_t mask[BYTES];
  if (!hexToBytes(maskHex, mask, BYTES))
    return false;
...
int convertRowLetterToNumber(const char *rowLetterReceived) {
  ...
  if (!rowLetterReceived || rowLetterReceived[0] == '\0')
    return -1;
...
void parseData() {
  ...
  long r = -1, g = -1, b = -1, bri = -1, times = -1;

  while (strtokIndx && tokenCount < 10) {
    tokens[tokenCount++] = strtokIndx;
    strtokIndx = strtok(NULL, ",");
  }
  // 根據 token 分類
  // 格式1: <A,1,C,barcode> (4 tokens)
  // 格式2: <A,1,C,barcode,timeMs> (5 tokens)
  // 格式3: <A,1,L,barcode,brightness> (5 tokens)
  // 格式4: <A,1,C,barcode,R,G,B> (7 tokens)
  // 格式5: <A,1,C,barcode,R,G,B,brightness> (8 tokens)
  // 格式6: <A,1,C,barcode,R,G,B,timeMs> (8 tokens)
  rowExposureTimeSpecified = false;
  if (tokenCount == 4 || tokenCount == 5 || tokenCount == 7 || tokenCount == 8) {
    strncpy(rowLetter, tokens[0], sizeof(rowLetter) - 1);
    rowLetter[sizeof(rowLetter) - 1] = '\0';
    columnNumber = atoi(tokens[1]);
    strncpy(illuminationCommand, tokens[2], sizeof(illuminationCommand) - 1);
    strcpy(illuminationCommand, tokens[2]);
    strncpy(plateBarcode, tokens[3], sizeof(plateBarcode) - 1);
    plateBarcode[sizeof(plateBarcode) - 1] = '\0';
    strcpy(plateBarcode, tokens[3]);
  } else {
    rowLetter[0] = '\0';
    columnNumber = 0;
    illuminationCommand[0] = '\0';  
    plateBarcode[0] = '\0';
  }
  if (tokenCount == 5) {
    times = strtol(tokens[4], NULL, 10);
    if (strcmp(illuminationCommand, "L") == 0) {
      bright = clamp8(times);
    } else if (commandSupportsRowExposure(illuminationCommand)) {
      rowExposureTimeMs = (uint32_t)times;
      rowExposureTimeSpecified = true;
    }
  }
  if (tokenCount == 7 || tokenCount == 8 || tokenCount == 9) {
    r=strtol(tokens[4], NULL, 10);
    g=strtol(tokens[5], NULL, 10);
    b=strtol(tokens[6], NULL, 10);
  ...
  if (tokenCount == 8){
    // 只有在 L 指令時才允許修改亮度 - 其他指令即使提供了亮度參數也會被忽略（不修改 bright 變數）
    if (strcmp(illuminationCommand, "L") == 0) {
      bri=strtol(tokens[7], NULL, 10); 
      bright = clamp8(bri);
    }else {
      times=strtol(tokens[7], NULL, 10);
      rowExposureTimeMs = (uint32_t)times;
      rowExposureTimeSpecified = true;
    }
  }
...
bool parseIlluminationCommand(const char *cmd) {
  ...
  if (strcmp(cmd, "X") == 0) { // Clear all LEDs
    clearDisplay();
    return true;
